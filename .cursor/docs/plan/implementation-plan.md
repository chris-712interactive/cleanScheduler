...
- **Gating:** `lib/billing/requireConnect.ts`; tenant shell `ConnectStatusBanner`; invoice detail **Pay with card** Checkout when `stripe_connect_status=complete`; manual form no longer lists **Card** (server rejects card method). Same gate for **subscription Checkout** and **customer portal invoice pay**. **Free trial:** Connect create + Checkout blocked. **New accounts (14d):** velocity caps via `lib/billing/connectPaymentVelocity.ts`.
